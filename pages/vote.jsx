...
const vote = ({ Web3Handler }) => {
  ...
  async function voteNFT(nft) {
    ...
    try {
      ...
      const hasVoted = await marketPlaceContract.methods
        .hasVoted(account)
        .call();
      if (hasVoted) {
        toast.error('You have already voted for a doodl!', {
          id: notification,
          style: {
            border: '2px solid #000',
          },
      ...
      // if user has not voted, allow them to vote
      const accounts = await web3.eth.getAccounts();
    ...
    <>
      ...
      <div className="flex justify-center">
        <p className="text-2xl font-bold text-center mt-4">
          <span className="text-red-500">reminder:</span> 1 vote per wallet
        </p>
      </div>
